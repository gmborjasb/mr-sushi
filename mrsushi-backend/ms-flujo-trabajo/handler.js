...
module.exports.completarEtapa = async (event) => {
  ...
  // Notificar al webhook de Rappi en GCP (Simulador Rappi Serverless)
  try {
    const RAPPI_WEBHOOK = "https://us-central1-rappi-api-501600.cloudfunctions.net/rappi-simulator/rappi/estado";
    console.log(`Enviando estado a Rappi: ${pedidoId} -> ${step} (Siguiente: ${nextStatus})`);
    
    // Convertimos nuestra notación interna (por ej. PACKING) al estado
    await axios.post(RAPPI_WEBHOOK, {
      pedidoId: pedidoId,
      estado: nextStatus
    }, { timeout: 3000 });
  } catch (error) {
    console.error("No se pudo notificar a Rappi:", error.message);
    // Ignoramos el error para no afectar el flujo interno si Rappi falla
  }
